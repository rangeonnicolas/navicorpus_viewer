// This code has mainly been taken from Raphaël Velt work concerning the graph visualisation and navigation:
...
// Some minor modifications have been made by Nicolas Rangeon for NaviCorpus
...
function displayNode(_nodeIndex, _recentre) {
    GexfJS.params.currentNode = GexfJS.params.activeNode;
    if (GexfJS.params.activeNode != -1) {
        addKeyWordAndUpdateView(GexfJS.graph.nodeList[GexfJS.params.activeNode].label, false);
    }
}
...
function updateWorkspaceBounds() {
    ...
    _tN.css({ 
        left :  $("#overviewzone").width()+6 +"px",
        //height:  $("#overviewzone").height()+"px",
    })

    var _ctl = $("#ctlzoom");

//NE PAS SUPPRIMER PARTIE SUIVANTE!!!!!!
/*    if(GexfJS.params.ctlZoomPosition == "right"){
        _ctl.css({
            bottom : $("#overviewzone").height()+5+"px" ,
            right : "5px"
        });
    }else if(GexfJS.params.ctlZoomPosition == "left"){
        _ctl.css({
            left: "5px", 
            bottom: "5px"
        });
    }else if(GexfJS.params.ctlZoomPosition == "top"){
        _ctl.css({
            top : "5px" ,
            right : "5px"
        });*/
        $("#zoomSlider").css({width: "120px" , height: "10px"});
        $("#zoomMinusButton").css({display: "inline-block"});
        $("#zoomPlusButton").css({display: "inline-block"});
        $("#lensButton").css({display: "inline-block"});
        $("#edgesButton").css({display: "inline-block"});
        $("#occurencies_pannel").css(
          {bottom:$("#textNavigation").height()+20+"px",
          left:GexfJS.overviewWidth+20+"px"});
...
function displayNodeCharac(_nodeIndex, _recentre) {
...
function loadGraph() {
    ...
    $.ajax({
        url: pathJoin([corporaPath,corpus,url]),
        dataType: "xml",
        success: function(data) {
            ...
            $(_nodes).each( function() {
                ...
                if(GexfJS.params.colorfulNodes!=false){
                  _maxCoul = Math.max(_r,_g,_b);
                  _minCoul = Math.min(_r,_g,_b);
                  _moyCoul = (_maxCoul + _minCoul)/2;
                  _eteCoul = _maxCoul - _minCoul;
                  if(_r == _maxCoul){ _r = 249;
                  }else if (_g == _maxCoul){ _g = 249;
                  }else if (_b == _maxCoul){ _b = 249;}
                  if(_r == _minCoul){ _r = 0;
                  }else if (_g == _minCoul){ _g = 0;
                  }else if (_b == _minCoul){ _b = 0;}
                  if       (_r != 0 & _r != 249){ _r = Math.floor((((_r - _moyCoul) / _eteCoul)*250)+125);
                  }else if (_g != 0 & _g != 249){ _g = Math.floor((((_g - _moyCoul) / _eteCoul)*250)+125);
                  }else if (_b != 0 & _b != 249){ _b = Math.floor((((_b - _moyCoul) / _eteCoul)*250)+125);}
                };
                _d.color = {
                    rgb : {
                        r : _r,
            ...
            $(_edges).each(function() {
                var _e = $(this),
                    ...
                    GexfJS.minEdgeWeight = Math.min(GexfJS.minEdgeWeight,_w);
...
function traceMap() {
    ...
    var _sizeFactor = GexfJS.echelleGenerale * Math.pow(GexfJS.echelleGenerale, -.15),
        ...
        _textSizeFactor = GexfJS.params.textSizeFactor;
    
    // Erasing the image of the graph zone
    GexfJS.ctxGraphe.clearRect(0, 0, GexfJS.graphZone.width, GexfJS.graphZone.height);
    ...
    for (var i in GexfJS.graph.nodeList) {
        ...
        if (_d.visible && _d.withinFrame) {
            if (i != _centralNode) {
                var _fs = _d.coords.real.r * _d.labelSize * _textSizeFactor;
                if (_d.isTag) {
                    if (_centralNode != -1) {
    ...
    GexfJS.ctxMini.strokeStyle = "rgb(150,150,150)";
...
function docready() {
  ...
  chargerCorpus(corpus, corporaPath);
    ...
    $("#aUnfold2")
        .click(onJoysticClick)
        .mousedown(startMoveJoys)
    $(document).mousemove(onJoysticMove).mouseup(function(){endMoveJoys();});
    $("#carte")
...
function docready_wrapper(){
	$("#howtouse_bckgnd").hide()
	docready();
}


if(first_time){
    $(document).ready(howtouse_screen1);
}else{
    $(document).ready(docready_wrapper);
}
